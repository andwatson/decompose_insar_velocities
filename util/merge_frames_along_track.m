...
% pre-allocate
track_vel = nan([size(xx) length(unique_tracks)]);
...
%                 vel(:,:,track_ind(jj+1)) = vel(:,:,track_ind(jj+1)) - m;
                vel(:,:,track_ind(jj+1)) = spfun(@(x) x-m,vel(:,:,track_ind(jj+1)));
...
%                 vel(:,:,track_ind(jj+1)) = vel(:,:,track_ind(jj+1)) - overlap_plane;
    ...
    if par.merge_tracks_along == 2
        ...
        if isempty(multi_segment)
            ...
            track_vel(:,:,ind_count) = mean(full_nan(vel(:,:,track_ind)),3,'omitnan');

            % merge component vectors
            track_compE(:,:,ind_count) = mean(full_nan(compE(:,:,track_ind)),3,'omitnan');
            track_compN(:,:,ind_count) = mean(full_nan(compN(:,:,track_ind)),3,'omitnan');
            track_compU(:,:,ind_count) = mean(full_nan(compU(:,:,track_ind)),3,'omitnan');

            % merge pixel uncertainties
            track_vstd(:,:,ind_count) = mean(full_nan(vstd(:,:,track_ind)),3,'omitnan');
            ...
            track_vel = cat(3,track_vel,nan([size(xx) n_seg-1]));
            track_compE = cat(3,track_compE,nan([size(xx) n_seg-1]));
            track_compN = cat(3,track_compN,nan([size(xx) n_seg-1]));
            track_compU = cat(3,track_compU,nan([size(xx) n_seg-1]));
            track_vstd = cat(3,track_vstd,nan([size(xx) n_seg-1]));
            ...
            for kk = 1:2:length(multi_segment)
                ...
                track_vel(:,:,ind_count+ind_count_inc) ...
                    = mean(full_nan(vel(:,:,track_ind(multi_segment(kk):multi_segment(kk+1)))),3,'omitnan');
                
                track_compE(:,:,ind_count+ind_count_inc) ...
                    = mean(full_nan(compE(:,:,track_ind(multi_segment(kk):multi_segment(kk+1)))),3,'omitnan');
                track_compN(:,:,ind_count+ind_count_inc) ...
                    = mean(full_nan(compN(:,:,track_ind(multi_segment(kk):multi_segment(kk+1)))),3,'omitnan');
                track_compU(:,:,ind_count+ind_count_inc) ...
                    = mean(full_nan(compU(:,:,track_ind(multi_segment(kk):multi_segment(kk+1)))),3,'omitnan');
                
                track_vstd(:,:,ind_count+ind_count_inc) ...
                    = mean(full_nan(vstd(:,:,track_ind(multi_segment(kk):multi_segment(kk+1)))),3,'omitnan');
