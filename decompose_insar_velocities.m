...
for ii = 1:nframes
    ...
    % mask all arrays with the velocities - this ensures that the
    % boundaries of the vels are the same (things like coastlines can mean
    % that e.g. compE extends further than vel).
    vstd_regrid_loop(isnan(vel_regrid_loop)) = nan;
    compE_regrid_loop(isnan(vel_regrid_loop)) = nan;
    compN_regrid_loop(isnan(vel_regrid_loop)) = nan;
    compU_regrid_loop(isnan(vel_regrid_loop)) = nan;
    mask_regrid_loop(isnan(vel_regrid_loop)) = nan;
...
asc_coverage = any(~isnan(full_nan(vel_regrid(:,:,asc_frames_ind))),3);
desc_coverage = any(~isnan(full_nan(vel_regrid(:,:,desc_frames_ind))),3);
...
coverage = sum(~isnan(full_nan(vel_regrid)),3);
